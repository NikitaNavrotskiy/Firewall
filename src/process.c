...
static void __fw_process_log_packet(FILE *fd_out, fw_packet_t *packet,
...
static bool __fw_process_regex_ip(regex_t *regexp, const char *ip) {
  bool ret = false;
...
static inline bool __fw_process_valid_proto(int proto) {
  return (proto == UDP || proto == TCP);
}

static inline bool __fw_process_valid_port(int port) {
  return port < 65536 && port > 0;
}

static int __fw_process_line_to_packet(const char *line, fw_packet_t *packet) {
  char src_ip[16] = {0};
  char dst_ip[16] = {0};
  int sport = 0;
  int dport = 0;
  ...
  status =
      sscanf(line, FW_PROCESS_FORMAT, src_ip, dst_ip, &sport, &dport, &proto);
  ...
  if (status != 5)
    ...
    if (!__fw_process_regex_ip(&regex_ip, src_ip) ||
        ...
        !__fw_process_valid_port(sport) || !__fw_process_valid_port(dport) ||
