 ...
 */
static inline enum fw_proto
__fw_rule_proto_from_str (const char *proto_str)
{
...
__fw_rule_action_from_str (const char *action_str)
{
 ...
 */
static bool
__fw_rule_str_regex_check (const char *str, regex_t *regex)
{
  int res;
  bool ret = false;
 ...
 */
static void
__fw_rule_make_offset (const char **str, int *last_offset)
{
  (*str) += *last_offset;
 ...
 */
static void
__fw_rule_retrieve_ip_net (char addr[19], uint32_t *ip, uint8_t *mask)
{
 ...
 */
static fw_rule_t *
__fw_rule_getline (fw_rule_t *last, const char *line, regex_t *regex)
{
 ...
 */
static bool
__fw_rule_ip_match (uint32_t ip, uint32_t net_addr, uint8_t subnet)
{
  if (net_addr == 0)
    return true;
...
static bool
__fw_rule_port_match (uint16_t checking_port, uint16_t rule_port)
{
  if (rule_port == 0)
    return true;
 ...
 */
static bool
__fw_rule_proto_match (enum fw_proto checking_proto, enum fw_proto rule_proto)
{
  if (rule_proto == NONE)
    return true;
 ...
 */
static bool
__fw_rule_packet_match (const fw_rule_t *rule, const fw_packet_t *packet)
{
...
fw_rule_t *
fw_rule_from_file (const char *file)
{
  regex_t regex = { 0 };
  FILE *fd;
  fw_rule_t *last = NULL;
  ...
  while (head == NULL && getline (&line, &len, fd) != -1)
    { // getting head
...
enum fw_action
fw_rule_process_packet (const fw_rule_t *rules, const fw_packet_t *packet)
{
