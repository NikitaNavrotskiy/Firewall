...
uint16_t gen_generate_port() { return rand() % 65535 + 1; }

enum fw_proto gen_generate_proto() { return rand() % 2 == 0 ? TCP : UDP; }

/**
 * @brief Function to generate one line for the output.
 *
 * @param fd_out Output stream.
 */
static void __gen_proc_line(FILE *fd_out) {
  char src_ip[16] = {0};
  char dst_ip[16] = {0};
